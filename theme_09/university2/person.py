...
class Person(object):
    ...
    def celebrate(self, what, with_whom):
        ...
        for person in with_whom:
            print('{0}{1}'.format(const.TAB, person))
...
class Employee(Person):
    ...
    def sign_contract(self, years=False, days=False):
        ...
        True
        >>> emp.end_work == emp.start_work + timedelta(3*const.YEAR)
        True
        ...
        if years is False and days is False:
            return None
        term = years.real * const.YEAR + days.real
...
contract
    ...
    def absent_work(self):
        ...
        'absent from work'
        """
        self.commit_violation(const.VIOLATIONS[const.ABSENT_FROM_WORK])
    ...
    def score_work(self, days):
        ...
        """
        self.commit_violation(
            const.VIOLATIONS[const.N_DAYS_ABSENT_FROM_WORK].format(days)
        )
...
language'
    ...
    def conduct_exam(self, lesson, group, limit=const.MAX_RATING):
        """
        >>> d_evm = Department('EVM', {})
        >>> teacher = Teacher('Ivanov Ivan', 3000, d_evm, age=30)
        >>> g_900502 = Group('Gr900502', {}, teacher)
        ...
        for student in group.consist['student']:
            if lesson not in student.exam:
                student.exam.append(
                    other.Exam(
                        lesson, random.randrange(const.MIN_RATING, limit), self
                    )
                )
    ...
    def conduct_retake(cls, exam):
        ...
        >>> d_evm = Department('EVM', {})
        >>> teacher = Teacher('Ivanov Ivan', 3000, d_evm, age=30)
        >>> exam = other.Exam('OPIP', 3, teacher)
        >>> teacher.conduct_retake(exam)
        >>> exam.rating >= 4
        True
        """
        exam.rating = random.randrange(const.POSITIVE_RATING, const.MAX_RATING)
    ...
    def set_automatic_rating(self, student, lesson):
        ...
        10
        """
        student.exam.append(other.Exam(lesson, const.MAX_RATING, self))
...
class Student(Person):
    ...
    def absent_lesson(self, lesson):
        ...
        self.violations.append(
            (const.VIOLATIONS[const.ABSENT_FROM_LESSON].format(lesson), today())
        )
    ...
    def score_lessons(self, days):
        ...
        """
        self.violations.append(
            const.VIOLATIONS[const.N_DAYS_ABSENT_FROM_LESSON].format(days)
        )
